...
action compound_action_1(src_addr, port_span, nhop_ipv4, port, dmac) {
    nop();
    nop();
    nat_int_to_ext(src_addr, port_span);
    set_nhop(nhop_ipv4, port);
    set_dmac(dmac);
}

action compound_action_2(dst_addr, port_span, nhop_ipv4, port, dmac){
    nop();
    nop();
    nat_ext_to_int(dst_addr, port_span);
    set_nhop(nhop_ipv4, port);
    set_dmac(dmac);
}
