...
action compound_action_int_to_ext(ipv4_addr, mac_addr, is_ext, srcAddr, nhop_ipv4, port, dmac) {
    set_if_info(ipv4_addr, mac_addr, is_ext);
    nat_hit_int_to_ext(srcAddr);
    set_nhop(nhop_ipv4, port);
    set_dmac(dmac);
}

action compound_action_ext_to_int(ipv4_addr, mac_addr, is_ext, dstAddr, nhop_ipv4, port, dmac) {
    set_if_info(ipv4_addr, mac_addr, is_ext);
    nat_hit_ext_to_int(dstAddr);
    set_nhop(nhop_ipv4, port);
    set_dmac(dmac);
}
...
table compound_table{
    ...
    actions {
        compound_action_int_to_ext;
        compound_action_ext_to_int;
    }
    size: 128;
}
...
control ingress {
    apply(compound_table) {
        miss {
            // retrieve information on the ingress interface into metadata
            apply(if_info);

            // determine whether to forward a packet
            // determine the right <address:port>
            apply(nat);
            ...
            if (meta.do_forward == 1 and ipv4.ttl > 0) {
                ...
                apply(ipv4_lpm);
                // rewrite destination ethernet address
                apply(forward);
            }
        }
    }
